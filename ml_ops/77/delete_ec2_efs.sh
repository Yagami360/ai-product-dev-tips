...
EFS_SECURITY_GROUP_ID=$( aws ec2 describe-security-groups --filter "Name=vpc-id,Values=${VPC_ID}" --query SecurityGroups[*].GroupId --output text | grep sg- )
EFS_FILE_SYSTEM_ID=$( aws efs describe-file-systems --query FileSystems[*].FileSystemId --output text )
...
# セキュリティーグループ
#if [ ${EFS_SECURITY_GROUP_ID} ] ; then
#	aws ec2 delete-security-group --group-id ${EFS_SECURITY_GROUP_ID}
#	echo "deleted security-group id=${EFS_SECURITY_GROUP_ID}"
#fi
...
EC2_SECURITY_GROUP_ID=$( aws ec2 describe-security-groups --filter "Name=vpc-id,Values=${VPC_ID}" --query SecurityGroups[0].GroupId --output text | grep sg- )
...
# セキュリティーグループ
#if [ ${EC2_SECURITY_GROUP_ID} ] ; then
#	aws ec2 delete-security-group --group-id ${EC2_SECURITY_GROUP_ID}
#	echo "deleted security-group id=${EC2_SECURITY_GROUP_ID}"
#fi
