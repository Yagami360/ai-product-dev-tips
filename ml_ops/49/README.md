# 【GCP】Cloud Monitoring（旧 Stackdriver Monitoring）に独自のメトリックを書き込む

> Cloud Monitoring と本項目での構成の説明。

<img src="https://user-images.githubusercontent.com/25688193/139069365-bdc1754d-e0fe-413b-a566-a0a830bb1165.png" width="1000"><br>
...
1. モニタリングサーバーのコード [api/monitoring-server/monitoring_server.py](https://github.com/Yagami360/MachineLearning_Tips/blob/master/ml_ops/49/api/monitoring-server/monitoring_server.py) を作成する<br>
    ...
    - Redis のキューに保管されている joib_id のリストを定期的に確認するために、バッチサーバーと同じく xxx

    - カスタム指標の Redis のキュー数は、整数値のデータなので、`descriptor.value_type = ga_metric.MetricDescriptor.ValueType.INT64` で整数値をしている。整数値を指定した場合は、`monitoring_v3.Point()` 
