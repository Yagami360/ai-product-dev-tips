...
# 関数 URL のエンドポイン
if [ $( aws lambda list-functions --query 'Functions[*].FunctionName' --output text | grep ${FUNCTION_NAME} ) ] ; then
    if [ $( aws lambda list-function-url-configs --function-name ${FUNCTION_NAME} --query 'FunctionUrlConfigs[*].FunctionUrl' --output text ) ] ; then
