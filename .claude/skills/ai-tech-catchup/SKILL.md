...
レポート本文ができたら、**ユーザーに確認を求めず、そのまま GitHub Issue を作成する**（このスキルは Claude Code Routines から無人で自律実行される前提）。

1. `references/report-formats.md` の「Issue 本文テンプレート」のヘッダ・フッタでレポート本文を囲み、一時ファイル（例 `/tmp/ai-tech-catchup-report.md`）に書き出す。使用モデルにはこのセッションのモデル名を入れる。
2. `gh` で投稿する。種別ラベル（`report` / `weekly-report` / `monthly-report` / `topic-report`）に加えて、**Claude Code Routines 経由であることを示す `claude-code-routine` ラベルを必ず付ける**。ラベルは未作成のことがあるので、無ければ作ってから付与する（冪等）:
   ...
   gh label create claude-code-routine --repo Yagami360/ai-product-dev-tips --color 5319e7 --description "Created via Claude Code Routines" 2>/dev/null || true
