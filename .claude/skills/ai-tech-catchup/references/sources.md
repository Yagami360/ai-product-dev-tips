...
このセッションで GitHub / Hugging Face / arXiv の MCP サーバーが使える場合は、Web 検索だけでなく以下の調査も行うと精度が上がる。使えない場合はスキップしてよい（無理に呼び出さず、`WebSearch` / `WebFetch` で代替する）。
...
### arXiv MCP サーバー
- `search_papers` で、キーワード／トピック・期間・カテゴリ（`cs.AI` / `cs.CL` / `cs.LG` / `cs.CV` / `cs.RO` など）を絞って最新論文を検索
- 重要な論文は `download_paper` / `read_paper` で全文を取得し、概要・先行研究との差分・新規性を把握
- 「研究者向けポイント」「技術トレンド」セクションの論文動向の裏取りに使う
