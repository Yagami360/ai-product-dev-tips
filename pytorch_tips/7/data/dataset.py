...
class DatasetType1(data.Dataset):
    def __init__(self, args, dataset_dir, datamode = "train", image_height = 128, image_width = 128, data_augument_types = "none", debug = False ):
...
class DatasetType2(data.Dataset):
    def __init__(self, args, dataset_dir, pairs_file = "train_pairs.csv", datamode = "train", image_height = 128, image_width = 128, data_augument_types = "none", debug = False ):
        super(DatasetType2, self).__init__()
        self.args = args
        self.dataset_dir = dataset_dir
        self.datamode = datamode
        self.data_augument_types = data_augument_types
    ...
    def __getitem__(self, index):
        image_s_name = self.df_pairs["image_s_name"].iloc[index]
        image_t_name = self.df_pairs["image_t_name"].iloc[index]
        self.seed_da = random.randint(0,10000)

        #---------------------
        # image_s
        #---------------------
        image_s = Image.open( os.path.join(self.dataset_dir, "image_s", image_s_name) ).convert('RGB')
        if not( "none" in self.data_augument_types ):
            set_random_seed( self.seed_da )
        ...
        #---------------------
        if( self.datamode == "train" or self.datamode == "valid" ):
            image_t = Image.open( os.path.join(self.dataset_dir, "image_t", image_t_name) ).convert('RGB')
            if not( "none" in self.data_augument_types ):
                set_random_seed( self.seed_da )
...
class DatasetType3(data.Dataset):
    def __init__(self, args, dataset_dir, datamode = "train", image_height = 128, image_width = 128, data_augument_types = "none", debug = False ):
        super(DatasetType3, self).__init__()
        self.args = args
        self.dataset_dir = dataset_dir
        self.datamode = datamode
        self.data_augument_types = data_augument_types
