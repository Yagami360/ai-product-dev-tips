...
# 自作モジュール
from data.dataset import DatasetType1, DatasetType2, DatasetType3
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_dir", type=str, default="dataset/dataset_type1")
    parser.add_argument('--dataset_type', choices=['type1','type2','type3'], default='type1' )
    ...
    elif( args.dataset_type == "type2" ):
        ds_train = DatasetType2( args, args.dataset_dir, pairs_file = "train_pairs.csv", datamode = "train", image_height = args.image_height, image_width = args.image_width, data_augument_types = args.data_augument_types, debug = args.debug )
        ds_valid = DatasetType2( args, args.dataset_dir, pairs_file = "valid_pairs.csv", datamode = "valid", image_height = args.image_height, image_width = args.image_width, data_augument_types = "none", debug = args.debug )
        dloader_train = torch.utils.data.DataLoader(ds_train, batch_size=args.batch_size, shuffle=True, num_workers = args.n_workers, pin_memory = True )
        dloader_valid = torch.utils.data.DataLoader(ds_valid, batch_size=args.batch_size_valid, shuffle=False, num_workers = 1, pin_memory = True )
    elif( args.dataset_type == "type3" ):        
        ds_train = DatasetType3( args, args.dataset_dir, datamode = "train", image_height = args.image_height, image_width = args.image_width, data_augument_types = args.data_augument_types, debug = args.debug )
