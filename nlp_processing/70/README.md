...
> **⚠️ VLM の出力は二峰性**: **ランダムに揺れるのではなく 2 つの答えを行き来する**。10 回の内訳は次のとおりで、cpu は **47.0 と -385.7 のコイン投げ**——同じ画像を見せても「当たり」と「大外れ」が半々になる。
>
> | センサー | 10 回の内訳（F1） |
> |---|---|
> | machine-temp | 2 つの答えを 6:4 で行き来 |
> | ambient-temp | 6:4（差は小さい） |
> | **cpu** | **2 つの答えを 5:5 で行き来（F1 0.667 と 0.50）** |
> | traffic-speed | 7:3 |
> | traffic-occupancy | 6:4 |
> | network | 6:4 |
>
> **2 回連続で同じ値が出ることも多いため、少数回の測定では安定していると誤認しやすい**（この Tip の執筆時も 2 回同値を見て「再現する」と誤判断し、3 回目で 80 点違う値が出て気づいた）。
...
- **誤検知率が高い**: 実測で平均 6.38%（(a) の 19 倍）、cpu では 13.7%。**異常が起きた「時間帯」を大まかに見つける用途には向くが、異常点をピンポイントで特定する用途には向かない**。ただし**誤検知イベント数で見れば少数**（machine-temp で 4.6 件）なので、人手確認を前提とする運用なら扱える。
