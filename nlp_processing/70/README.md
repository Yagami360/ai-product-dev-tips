...
この設定では画像化→VLM が全区間を検出し最高の recall で、「数値テキストより画像の方が頑健」という研究とも一致する。ただし上表を「画像化→VLM が TSFM+LLM より優れている」と一般化してはいけない。

- **学術的にも「どの手法も万能に最良」ではない**: ベンチマーク mTSBench は「どの検出器も全データセットで優位に立てない」、TSB-AD（NeurIPS 2024）は「単純な統計手法がしばしば深層学習を上回り、基盤モデル系（Chronos 等）は**点異常で有望**」と報告。
- **この設定は (c) TSFM に不利**: 公平化のため系列を粗く間引いた（`--downsample 24` / 946 点）ため、Chronos が得意な**細かい時間構造が潰れ**検出力が下がる。[67](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/67) の細かい設定（downsample 6）では (c) はより強い。
- **指標が「広く当てる」方式に有利**: `window_recall` と `PA-F1` は区間内に 1 点でも当たれば高得点なので、広い時間帯を返す本方式 (b) が有利に出る（PA-F1 は水増し傾向。本来は VUS-PR / PATE 等が望ましい）。
- **検知精度しか測っていない**: (c) TSFM+LLM の価値は「検知＋説明＋コスト＋商用実証の両立」にあり、説明品質（[67](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/67) の LLM-as-judge で 5/5）・コストは未計測。検知 F1 だけでは (c) を過小評価する。
- **本方式 (b) 固有の弱点**: **季節性異常に弱い**（TAMA で 29.0%）ため、周期性の強い別データでは順位が入れ替わりうる。検出範囲も広く誤検知が増える。

→ まとめると、**(c) TSFM+LLM が「検知＋説明のバランス」で実務的に有力**という位置づけは変わらない。本表は「検知単独・単一データ・粗い設定・甘い指標での一例」として読むこと。
