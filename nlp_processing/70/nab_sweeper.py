"""NAB 公式スコアラー（Numenta Anomaly Benchmark の sweeper.py をそのまま流用）。

出典: https://github.com/numenta/NAB の nab/sweeper.py（MIT, Copyright 2014-2024 Numenta Inc.）
NAB 公式スコアは「検出の早さ」を評価する採点方式で、正解の異常区間の
早い時点で検知するほど高得点、区間を逃すと FN ペナルティ、
区間外の検知は FP ペナルティ（区間終端からの距離で減衰）となる。
本ファイルは改変せずそのまま用いる（採点ロジックを自作すると公式値と乖離するため）。
"""
