...
| センサー（データ数） | 検知結果（<span style="color:#ff7f0e">■</span> 帯＝NAB が定義する異常区間／<span style="color:#d62728">●</span>＝LLM の検知点。クリックで原寸） | スコア（N=10平均） |
|---|---|---|
| `machine-temp` (946) | <a href="images/machine-temp_numeric_llm.png"><img width="240" src="images/machine-temp_numeric_llm.png" /></a> | 検出率=50.00% / 適合率=83.00%<br>**F1=0.619**<br><sub>誤検知率=0.06%</sub> |
| `ambient-temp` (606) | <a href="images/ambient-temp_numeric_llm.png"><img width="240" src="images/ambient-temp_numeric_llm.png" /></a> | 検出率=100.00% / 適合率=100.00%<br>**F1=1.000**<br><sub>誤検知率=0.00%</sub> |
| `cpu` (672) | <a href="images/cpu_numeric_llm.png"><img width="240" src="images/cpu_numeric_llm.png" /></a> | 検出率=50.00% / 適合率=100.00%<br>**F1=0.667**<br><sub>誤検知率=0.00%</sub> |
| `traffic-speed` (564) | <a href="images/traffic-speed_numeric_llm.png"><img width="240" src="images/traffic-speed_numeric_llm.png" /></a> | 検出率=75.00% / 適合率=54.00%<br>**F1=0.627**<br><sub>誤検知率=0.51%</sub> |
| `traffic-occupancy` (1190) | <a href="images/traffic-occupancy_numeric_llm.png"><img width="240" src="images/traffic-occupancy_numeric_llm.png" /></a> | 検出率=100.00% / 適合率=25.00%<br>**F1=0.400**<br><sub>誤検知率=0.28%</sub> |
| `network` (789) | <a href="images/network_numeric_llm.png"><img width="240" src="images/network_numeric_llm.png" /></a> | 検出率=100.00% / 適合率=26.00%<br>**F1=0.400**<br><sub>誤検知率=1.15%</sub> |

**検出は総じて疎**（2〜15 点）で、そのぶん**誤検知率が全センサーで 1.15% 以下**と極めて低い（適合率は cpu で 1.00、machine-temp で 0.83）。代償として**見逃しがある**（machine-temp・cpu の検出率 0.50）。図は **10 回のうち平均に最も近い試行**を掲載。
...
| センサー（データ数） | 数値直接入力（本 Tip）<br>スコア（N=10平均） | 画像→VLM（[70](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/70)）<br>スコア（N=10平均） | TSFM Chronos（[67](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/67)）<br>決定的（N=1 で十分） |
|---|---|---|---|
