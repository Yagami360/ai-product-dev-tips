...
主指標は **NAB 公式スコア**（`standard` プロファイル）。NAB 公式スコアラー（[`nab_sweeper.py`](nab_sweeper.py)）で採点し、**異常窓の早い位置で検知するほど高得点**、窓の見逃しは FN、窓外の検知は FP としてペナルティを課す。**100 = 完璧、0 = 無検出と同等、マイナス = 無検出より悪い**。
