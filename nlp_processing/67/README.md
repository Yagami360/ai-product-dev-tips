...
- **Chronos はセンサーデータでも使える（汎用 TSFM）**: Chronos は特定ドメイン専用ではなく、事前学習コーパスに energy（電力）・weather（気象）・traffic（交通）・sensor（センサー）など多様なドメインを含む汎用の時系列基盤モデルで、未知の系列にゼロショット転移できる（[Amazon Science](https://www.amazon.science/blog/adapting-language-model-architectures-for-time-series-forecasting) / [論文 arXiv:2403.07815](https://arxiv.org/abs/2403.07815)）。産業センサーの異常検知に適用した研究（ChronosAD, [arXiv:2606.01300](https://arxiv.org/abs/2606.01300)、製造業向け）や、ベアリング故障データ（CWRU）での適用例もあり、**IoT・設備センサー領域で実際に使われている**。オブザーバビリティ領域では Datadog が同型構成を商用稼働させている。
- **TSFM は本来「予測」用**: Chronos は forecasting モデルであり、異常検知はその予測残差を使う転用。周期性の強いセンサー系列（温度・電力・振動のトレンド等）では有効だが、高周波の生振動波形や複雑な多変量異常・微細な異常には、異常検知を公式サポートする TSFM（IBM `granite-timeseries-tspulse` 等）や専用手法の方が向く場合がある。
