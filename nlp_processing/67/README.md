...
同じパイプラインを全センサーで実行した結果（間引きは後述の「④ 3 系統の公正な精度比較」と同一設定・しきい値はセンサー別。Chronos は決定的なので N=1 で十分）。

| センサー（データ数） | 検知結果（<span style="color:#ff7f0e">■</span> 帯＝NAB が定義する異常区間／<span style="color:#d62728">●</span>＝Chronos の検知点。クリックで原寸） | スコア |
|---|---|---|
| `machine-temp` (946) | <a href="images/machine-temp_anomaly.png"><img width="240" src="images/machine-temp_anomaly.png" /></a> | 検出率=50.00% / 適合率=29.00%<br>**F1=0.364**<br><sub>誤検知率=0.59%</sub> |
| `ambient-temp` (606) | <a href="images/ambient-temp_anomaly.png"><img width="240" src="images/ambient-temp_anomaly.png" /></a> | 検出率=0.00% / 適合率=0.00%<br>**F1=0.000**<br><sub>誤検知率=0.00%</sub> |
| `cpu` (672) | <a href="images/cpu_anomaly.png"><img width="240" src="images/cpu_anomaly.png" /></a> | 検出率=50.00% / 適合率=33.00%<br>**F1=0.400**<br><sub>誤検知率=0.60%</sub> |
| `traffic-speed` (564) | <a href="images/traffic-speed_anomaly.png"><img width="240" src="images/traffic-speed_anomaly.png" /></a> | 検出率=50.00% / 適合率=67.00%<br>**F1=0.571**<br><sub>誤検知率=0.20%</sub> |
| `traffic-occupancy` (1190) | <a href="images/traffic-occupancy_anomaly.png"><img width="240" src="images/traffic-occupancy_anomaly.png" /></a> | 検出率=100.00% / 適合率=4.00%<br>**F1=0.080**<br><sub>誤検知率=2.33%</sub> |
| `network` (789) | <a href="images/network_anomaly.png"><img width="240" src="images/network_anomaly.png" /></a> | 検出率=100.00% / 適合率=6.00%<br>**F1=0.114**<br><sub>誤検知率=4.65%</sub> |
| **全体平均** | — | **F1=0.255**<br><sub>誤検知率=1.40%</sub> |
...
**予測モデルの転用ゆえ検知は弱め**。`ambient-temp` は間引き後の系列で予測区間を超えられず**検知 0**（F1 0.000）、`traffic-occupancy` / `network` は検出はするが適合率が壊れる（0.04 / 0.06）。**「TSFM だから検知精度が高い」とは言えない**。

### ⚠️ しきい値がセンサーごとに要調整

`make run NAB_KEY=...` を変えるだけで温度以外のセンサーでも動く（各センサーの plot と `reports/<センサー>.md` を生成）が、**異常スコアのスケールがセンサーで桁違い**（実測の最大値は `ambient-temp` が 1.22、`network` が 660）。予測区間幅で正規化してもこの差が残るため、一律 1.5 だと `ambient-temp` / `traffic-occupancy` は**検知 0 件**になる。そこで [`Makefile`](Makefile) にセンサー別のしきい値（`THRESHOLD_<センサー>`）を持たせている。

| センサー | 既定しきい値 | スコア最大（実測） |
|---|---|---|
| `machine-temp` | 1.5 | 9.36 |
| `ambient-temp` | **0.8** | 1.22 |
| `cpu` | **0.5** | 2.40 |
| `traffic-speed` | 1.5 | 2.87 |
| `traffic-occupancy` | **0.6** | 1.15 |
| `network` | 1.5 | **660.37** |

**しきい値が要調整であることは本方式の実務上の弱点**。`network`（スコア 660）と `ambient-temp`（1.22）が混在するため、**一律のしきい値では運用できない**——新しいセンサーごとに調整が要る。
...
| ambient-temp | 5 | 5.0 |
| cpu | 4 | 4.75 |
| traffic-speed | 4 | 4.75 |
| traffic-occupancy | 4 | 4.75 |
| network | 4 | 4.75 |
| **平均** | — | **4.83** |

**忠実性が一度も 1〜2 に落ちない**（`temperature=0.0`）。**4 系統すべて同一のプロンプト**・同じ審査基準・同じ LLM（`gemini-3.5-flash`）での比較で、系統 (a)（[69](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/69)）は平均 4.46（忠実性 1〜2 が 2 回）、系統 (b)（[70](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/70)）は 3.96（同 4 回）だったのと対照的。なお検知層を TSPulse に替えた [71](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/71) は **4.96** とさらに高い。
...
>
> - **レポート品質は TSFM 系が優位**: LLM-as-judge で (c) Chronos 4.83 / **(d) TSPulse 4.96**（[71](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/71)）に対し、(a) 4.46 / (b) 3.96。**4 系統すべて同一プロンプト・同一モデル・同一 temperature での差**なので、違いは **LLM に渡す事実の粒度**だけに起因する。**忠実性が 1〜2 に落ちた回数は TSFM 系が 0 / 6**、(a) が 2 / 6、(b) が **4 / 6**。
...
- **検知精度は 3 系統中で最下位**: 実測（上記 ④）で F1 平均 0.255 と、数値直接入力（0.619）・画像→VLM（0.588）に大きく劣る。**適合率が壊滅的**（traffic-occupancy 0.04、network 0.06）で、異常は見つけるが大量の誤検知に埋もれる。本方式の価値は検知精度ではなく、**検証可能な事実を LLM に渡して幻覚を封じる**点にある（レポート品質は judge 平均 4.83 で、忠実性が一度も 1〜2 に落ちない）。
