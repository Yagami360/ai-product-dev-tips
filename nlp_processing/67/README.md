...
主指標は **NAB 公式スコア**（`standard` プロファイル）。NAB 公式スコアラー（[`nab_sweeper.py`](nab_sweeper.py)）で採点し、**異常区間の早い時点で検知するほど高得点**。異常区間を 1 つも検知できなければ FN、異常区間の外での検知は FP としてペナルティを課す。**100 = 完璧、0 = 無検出と同等、マイナス = 無検出より悪い**。
...
| センサー（データ数） | 数値直接入力（[69](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/69)）<br>N=10 平均 | 画像→VLM（[70](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/70)）<br>N=10 平均 | TSFM Chronos（本 Tip）<br>決定的（N=1 で十分） |
|---|---|---|---|
...
- **評価指標の罠**: 時系列異常検知は point-adjust F1 による過大評価が問題視されており、本 Tip でも実測で再現した（[70](https://github.com/Yagami360/ai-product-dev-tips/tree/master/nlp_processing/70) の traffic-occupancy は PA-F1 0.891 に対し NAB スコア -40.6）。そのため主指標には **NAB 公式スコア**（NAB データの公式採点方式）を採用している。なお VUS-PR / PATE 等の閾値フリー指標は**連続スコアを要求する**ため、検知結果が離散的な系統 (a)(b) には適用できず、3 系統の比較には使えない。
