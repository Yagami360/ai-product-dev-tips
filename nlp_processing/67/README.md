...
> **この 2 段アーキテクチャの商用実証について（正確な範囲）**: 「TSFM で検知 → LLM で説明」という構成そのものは、**Datadog が IT オブザーバビリティ（サーバ/インフラ/APM/ネットワーク等のメトリクス監視）領域で商用稼働**させている（自社 TSFM [Toto](https://github.com/DataDog/toto) は「**observability metrics 専用**」の基盤モデル、LLM エージェント [Bits AI SRE](https://www.datadoghq.com/blog/bits-ai-sre-deeper-reasoning/) が根本原因調査を自動化）。ただし **Datadog が扱うのは物理 IoT センサーではなく IT メトリクス（ソフトウェアのテレメトリ）**である点に注意。本 Tip が扱う**製造・設備系の物理センサー**への同型構成の適用は、アーキテクチャは同じだが商用化はまだ進んでおらず、研究段階（調査ドキュメント §4.4 でも「物理センサー領域は差別化余地」と整理）。
