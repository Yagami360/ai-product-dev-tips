# CoVe [Chain-of-Verification] を使用して LLM のハルシネーションを低減する
...
1. Ollama をインストールして起動する
    ...
    [Ollama 公式サイト](https://ollama.com/)からインストールする。Ollama は量子化済みモデルを GPU 不要（CPU のみ）で動かせるローカル LLM ランタイムで、起動すると `http://localhost:11434` で API を待ち受ける。
...
<!-- TODO: 実行結果のスクショ or 出力ログを貼り付け（CoVe あり／なしの比較） -->

実行イメージは以下の通り（出力は使用するモデル・実行時点により変わるため、あくまで一例）。

```text
[Step 1] Baseline Response:
  （初期回答。事実列挙のうち一部の受賞年などに誤りが混入することがある）
------------------------------------------------------------
[Step 2] Verification Questions:
  - 〇〇がノーベル物理学賞を受賞したのは何年か？
  - △△はノーベル物理学賞を受賞しているか？
  ...
------------------------------------------------------------
[Step 3] Verification Answers (factored):
  Q: 〇〇がノーベル物理学賞を受賞したのは何年か？
  A: （初期回答を文脈に含めず、独立して回答するので誤りを引き継ぎにくい）
  ...
------------------------------------------------------------
[Step 4] Final Verified Response:
  （検証結果と矛盾する受賞年を訂正し、確認できなかった人物を除いた最終回答）
```

検証の観点は以下の通り。

- **初期回答（baseline）に含まれる誤り**（誤った受賞年・実在しない人物など）が、最終回答（cove）で訂正・除去されているか。
- factored 方式では Step 3 の検証回答が初期回答に引きずられにくいため、誤りを発見しやすい。
- 一方で、4 ステップ＋検証質問数ぶんの LLM 呼び出しが発生するため、`baseline` に比べて応答が遅くなる（CPU では特に顕著）。

> **軽量モデルでの実測メモ**: 上記コードを `qwen3.5:2b`（CPU）で動かすと、CoVe の 4 ステップ自体は問題なく流れる。ただし `2b` のような小型モデルは、難しい closed-book な事実想起（例: ノーベル賞受賞者と受賞年）では **baseline だけでなく検証回答（Step 3）も誤りやすく**、CoVe をかけても誤りが訂正されないことがある（検証が間違った事実を「正しい」と追認してしまう）。CoVe の訂正効果をはっきり見たい場合は、`qwen3.5:4b` 以上や、より大きなモデルを使うとよい。CoVe は「検証ステップがある程度正確に答えられる能力」を前提とする技法である点に注意。
