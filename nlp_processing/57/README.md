...
1. Qwen3 系の最新軽量モデル（Qwen3.5）を取得する
    ...
    | `qwen3.5:2b` | 約 2.7GB | CPU で実用的な速度 × 軽さのバランス（推奨） |
    | `qwen3.5:4b` | 約 3.4GB | 2b より検証品質は上がるが、CoVe の多段呼び出しで 1 回答に時間がかかる |
    | `qwen3.5:9b` | 約 6.6GB | CPU で動かせる上限の目安（RAM 16GB 程度）。検証の正確さは最も高いが 1 回答は遅い |

    > `--model` 引数（argparse）でモデルを切り替えられる（例: `--model qwen3.5:9b`）。モデルサイズによる CoVe 効果の違いは後述の「[モデルサイズ別の A/B 比較](#モデルサイズ別の-ab-比較--model-で切り替え)」を参照。
...
### モデルサイズ別の A/B 比較（`--model` で切り替え）

使用モデルは `run_cove.py` の `--model` 引数（argparse）で切り替えられる。より大きく CPU で動かせる `qwen3.5:9b`（約 6.6GB、RAM 16GB 程度で動作）でも同じプロンプトを試した結果が以下（`qwen3.5:4b` も併記）。

```sh
# 例: より大きいモデルで実行（CPU のみ・GPU 不要）
python3 run_cove.py --model qwen3.5:9b --mode cove --prompt "ノーベル物理学賞を受賞した日本人を3人挙げて、それぞれの受賞年も教えて。簡潔に。"
```

| モデル | baseline（CoVe なし） | cove（CoVe あり・最終回答） | 判定 |
|--------|----------------------|-----------------------------|------|
| `qwen3.5:2b` | 池田俊夫/大隅良典/梶原隆司（全て誤り） | 田中耕一/大隅良典/梶原隆司（全て誤り） | ❌ 訂正されず |
| `qwen3.5:4b` | 小柴昌俊1997/梶田隆章2002/田中耕一2011（誤り） | 小柴昌俊1989/梶田隆章2002/田中耕一2008（誤り） | ❌ 訂正されず |
| `qwen3.5:9b` | **湯川秀樹1949/小柴昌俊2002/梶田隆章2015（正解）** | 同上（正解を維持） | ⭕ baseline から正確 |

ここから分かる重要な傾向は次の通り。

- **CoVe の効果はモデルの能力（特に検証ステップの正確さ）に強く依存する**。小型（2b/4b）では Step 3 の検証回答自体が誤るため CoVe は誤りを正せず、大型（9b）では baseline がそもそも正確で CoVe の出番が少ない。「baseline は誤るが検証は正しく答えられる」というスイートスポットは、モデル単体では意外と狭い。これは論文が大型モデル（Llama 65B）で大きな効果を示したことと整合する。

- **CoVe の機構が観察できる例（誤った前提の棄却）**: `qwen3.5:9b` に誤った前提を含む質問「夏目漱石の代表作『走れメロス』のあらすじを…」を投げると、CoVe は Step 3 の独立検証を経て、最終回答で「**夏目漱石ではなく**（別の作家）の作品」と前提の誤りを明示的に棄却した（独立検証が効いた例）。ただし正しい作者「太宰治」には辿り着けず別の作家名を誤って挙げており、**検証もモデルの知識に縛られる**点は変わらない。
...
> **実測のまとめ**: 上記の通り、ローカルの Qwen3.5 系（2b〜9b）を CPU で動かす範囲では、難しい closed-book な事実想起（例: ノーベル賞受賞者と受賞年）に対して **CoVe は安定した訂正効果を示さなかった**。小型モデルは検証回答（Step 3）自体が誤り、大型モデルは baseline がそもそも正確になるためである。CoVe は「**検証ステップがある程度正確に答えられる能力**」を前提とする技法であり、ローカル小型モデル単体では限界がある。論文の大きな効果（Llama 65B）に近づけるには、**より大規模なモデルを使う**か、**検証ステップに検索（RAG）・外部ツールを組み合わせて検証回答の事実性を担保する**のが現実的（後述）。CoVe の機構そのものを軽量モデルで観察したい場合は、上記「誤った前提の棄却」のような、モデルが元々知っている常識レベルの事実を問うシナリオの方が分かりやすい。
