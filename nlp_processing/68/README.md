...
- [センサー信号を LLM に接続する 5 系統の中での位置づけ](#-センサー信号を-llm-に接続する-5-系統の中での位置づけ)
- [SensorLLM の 2 段構成（アーキテクチャ）](#-sensorllm-の-2-段構成アーキテクチャ)
- [公開状況の整理（何が提供され、何が無いか）](#-公開状況の整理何が提供され何が無いか)
- [必要リソース（事前チェックリスト）](#-必要リソース事前チェックリスト)
- [使用方法](#-使用方法)
...
- **GPU**: 学習は **Ampere 以降（A100 等）が必須**（bf16 + flash-attn 前提）。flash-attn 2 のプリビルド wheel（torch2.4/CUDA12/cp311）を `Dockerfile` で焼き込むため**ソースビルドは不要**。**推論は flash-attn 不要**で、`--dtype float16` なら T4/V100 でも動く。VRAM 目安: 小型 1.1B ≒ 8〜16GB、Llama-3-8B ≒ 24GB。
- **ベース LLM**: LLaMA 系（HF で利用同意の上 DL）。既定 `--fix_llm True` で重み固定。
- **時系列エンコーダ**: Chronos の ckpt（公開。`--pt_encoder_backbone_ckpt` に指定）。
- **データ**: 対応 5 データセット（USC-HAD / UCI-HAR / MHealth / Capture-24 / PAMAP2）のいずれか（本 Tip は MHealth）。他データセットは [`ts_backbone.yaml`](https://github.com/cruiseresearchgroup/SensorLLM/blob/main/sensorllm/model/ts_backbone.yaml) と [`sensorllm/data`](https://github.com/cruiseresearchgroup/SensorLLM/tree/main/sensorllm/data) の調整が必要。
