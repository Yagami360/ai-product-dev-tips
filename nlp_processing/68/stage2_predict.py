...
def main():
    ...
    ap.add_argument("--plot", default=None,
                    help="指定パスに、全サンプルを 1 枚に並べたグリッド図を保存")
    ap.add_argument("--plot-dir", default=None,
                    help="指定ディレクトリに、1 サンプルごとに独立した plot 図を保存")
    ap.add_argument("--save-results", default=None,
                    help="推論結果（センサー窓＋pred/true）を npz に保存（後でモデル無しで再プロット可）")
    ap.add_argument("--from-results", default=None,
                    help="保存済み npz からプロットのみ行う（推論・モデル読込をスキップ）")
    args = ap.parse_args()
...
def load_results(path):
    ...
    samples = [(int(d["idxs"][k]), d["windows"][k], int(d["preds"][k]),
                int(d["trues"][k]), bool(d["oks"][k])) for k in range(len(d["idxs"]))]
...
def _acc_magnitudes(window, is_mhealth):
    ...
    return [(name, color, np.sqrt(sum(window[j] ** 2 for j in idx)))
            for name, color, idx in MHEALTH_ACC_MAG]
...
def save_single_plots(samples, id2label, out_dir):
    ...
    for i, window, pred_id, true_id, ok in samples:
        ...
        if is_mhealth:
            ...
            C = window.shape[0]
            ncol = min(4, C); nrow = math.ceil(C / ncol)
            fig, axes = plt.subplots(nrow, ncol, figsize=(3 * ncol, 2 * nrow), squeeze=False)
            for k in range(nrow * ncol):
                ax = axes[k // ncol][k % ncol]
                if k < C:
                    ax.plot(window[k], lw=1.0); ax.set_title(f"ch{k}", fontsize=8)
                    ax.tick_params(labelsize=6)
                else:
                    ax.axis("off")
        fig.suptitle(f"sample #{i}  [{mark}]   pred: {_short(id2label[pred_id])}   /   "
                     f"true: {_short(id2label[true_id])}", fontsize=12, color=edge)
        for spine in fig.get_axes()[0].spines.values():
            pass  # サブプロット個別の枠は付けず、figure 全体の枠色で正誤を示す
        fig.patch.set_edgecolor(edge); fig.patch.set_linewidth(4)
...
def save_grid_plot(samples, id2label, correct, n, path):
    ...
    fig.suptitle(f"SensorLLM Stage2 HAR classification  |  panel = one 2s test window; "
                 f"lines = acceleration magnitude |acc| at 3 body sites; "
                 f"frame green=correct / red=wrong; title pred / true  |  "
                 f"Accuracy {correct}/{n}={correct / n:.3f}", fontsize=9)
