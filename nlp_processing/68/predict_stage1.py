...
def parse_args():
    ...
    parser.add_argument(
        "--model-path", type=str, default="1EE1/SensorLLM-Stage1-Backup", help="Stage1 チェックポイント(既定は非公式の MHealth 学習済み重み)"
    )
    parser.add_argument(
        "--chronos-path",
        type=str,
    ...
    parser.add_argument("--dataset", type=str, default="mhealth", help="ts_backbone.yaml のデータセット名(既定 ckpt は mhealth で学習)")
    parser.add_argument("--channel", type=str, default=None, help="センサーチャネル名(未指定なら当該データセットの先頭チャネル)")
    parser.add_argument("--tokenize-method", type=str, default="StanNormalizeUniformBins", help="Chronos のトークン化方式(学習時と揃える)")
    parser.add_argument("--dtype", type=str, default="bfloat16", choices=list(DTYPE_MAP), help="T4/V100 では float16 を指定する")
    parser.add_argument("--device", type=str, default="cuda", choices=["cuda", "cpu"])
    parser.add_argument("--input", type=str, default=None, help="1 次元センサー系列の .npy パス(未指定なら合成波形を生成)")
    parser.add_argument("--length", type=int, default=200, help="合成波形の系列長")
    parser.add_argument("--question", type=str, default="What is the overall trend of this sensor reading?", help="モデルへの質問")
...
def load_model(args, dtype):
    ...
    model = SensorLLMStage1LlamaForCausalLM.from_pretrained(args.model_path, low_cpu_mem_usage=False, use_cache=False, torch_dtype=dtype).to(
        args.device
    )

    # 時系列エンコーダ(Chronos)のバックボーンをロード
    model.get_model().load_pt_encoder_backbone_checkpoint(args.chronos_path, tc=args.tokenize_method, torch_dtype=dtype)
...
def build_sensor_series(args):
    ...
    if args.input:
        ...
        series = (np.sin(t) + 0.3 * np.sin(3 * t) + 0.02 * t + 0.05 * np.random.randn(args.length)).astype(np.float32)
...
if __name__ == "__main__":
    ...
    inputs, ts_token_ids, ts_attention_mask = build_prompt(args, model, tokenizer, chronos_tokenizer, series)
